...
*/
class Solution {
    static Node findrandom(Node root,Node random){
        Node temp = root;
        
        while(temp!=null){
            if(temp.data == random.data) return temp;
            temp = temp.next;
        }
        return null;
    }
    public Node cloneLinkedList(Node head) {
        ...
        // Step 1: Create new cloned nodes and interleave them with the original nodes
        while (temp != null) {
        ...
        // Step 2: Update the random pointers in the new cloned nodes
        temp = head;
        while (temp != null) {
            if (temp.random != null) {
                temp.next.random = temp.random.next;  // The cloned node's random pointer should point to the clone of temp.random
            }
        ...
        temp = head;
        Node nhead = head.next;  // Head of the cloned list
        Node copy = nhead;

        while (temp != null) {
            temp.next = temp.next.next;  // Restore the original list
            if (copy.next != null) {
                copy.next = copy.next.next;  // Update the cloned list
            }
            temp = temp.next;
            copy = copy.next;
        }
